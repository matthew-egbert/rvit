...
class Tracker3D(xyz_bounds,color,gradient):
    ...
    z_scalar = StringProperty('') #: variable which is to be tracked
    z_scalar_preprocess = StringProperty('') #: the preprocessor
    ...
    def registerConfigurableProperties(self):
        ...
        self.addConfigurableProperty(Tracker3D.num_samples,rank = 5)
        self.addConfigurableProperty(Tracker3D.line_width, rank = 5)
    ...
    def on_num_samples(self, obj, value):
        self.N = int(value)
        self._index = 0
        self.data = np.zeros((self.N, 3),dtype=np.float32)
    ...
    def on_z_scalar(self, obj, value):
        if not hasattr(self,'simulation'):
            self.simulation = App.get_running_app().get_simulation()
        if value != '':
            self.get_z_command = 'self._z = self.simulation.%s' % (value)
            exec(self.get_z_command)
        if hasattr(self,'preprocess') :
            self._z = self.preprocess_y(self._z)
        self.loadShaders()

    def on_z_scalar_preprocess(self, obj, value):
        s = 'self.preprocess_z = %s' % (value)        
        exec(s)
    ...
    def update(self):
        ...
        if hasattr(self,'get_z_command'):
            exec(self.get_z_command)
        else:
            raise Exception('No z_scalar has been specified or there was an error in loading it.')

        if hasattr(self,'preprocess_x') :
            self._x = self.preprocess_x(self._x)
        if hasattr(self,'preprocess_y') :
            self._y = self.preprocess_y(self._y)
        if hasattr(self,'preprocess_z') :
            self._z = self.preprocess_z(self._z)
        self.data[self._index,0] = self._x
        self.data[self._index,1] = self._y
        self.data[self._index,2] = self._z
    ...
    def loadShaders(self):
        ...
        self.shaders = glsl_utils.loadShaders('tracker3d.glsl',self.shader_substitutions)
